...
Create a vector called `output` that is filled with `NA` values and has length `n`. We've already loaded `n` into your environment with a value of 10. Hint: the `rep()` function may be useful here!
...
Use the `str_c()` function to put together a phrase and the object `n`. The final output should read `The object n has value 10`. 
...
```{r cat-solution}
str_c("The object n has value ", n)
```
