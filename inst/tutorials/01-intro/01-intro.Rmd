---
title: "QSS Tutorial 0: Introduction to R"
tutorial:
  id: "00-intro"
output:
  learnr::tutorial:
...
tidyverse generally uses aptly-named functions instead of base R code. Here, instead of using brackets or dollar signs, we'll begin by using `slice()` and `select()`
...
We'll commonly be using conditional statements to subset -- telling R to subset the data based on whether it's equal to (==), greater than (>), greater than or equal to (>=), less than (<), less than or equal to (<=), or not equal to (!=) a certain value. Note that you need to use two equal signs for equal to -- R treats one equal sign as an assignment similar to `<-`, so your subsetting will not work
...
UNpop %>% 
  mutate(century = ifelse(year < 2000, "1900s", "2000s")) %>%
  group_by(century) %>%
  summarize(mean.century.pop = mean(world.pop))
```
